...
def on_baecha_complete():
    ...
    for taxi_id, cluster in result.items():
        ...
        notify_taxi_baecha_result(taxis[taxi_id],
                                  (np.float64(cluster.y_axis),
                                   np.float64(cluster.x_axis)),
                                  cluster.description,
                                  cluster.predicted_demand,
                                  cluster.predicted_reason)
...
def notify_dashboard_baecha_result(result: AssignmentResult,
    ...
    for taxi_id, cluster in result.items():
        ...
        # disconnect된 taxi가 정상적으로 지워지지 않아 임시 방편으로 무시.
        # on_disconnect가 제대로 작동하지 않는 듯.
        if taxi_id not in taxis:
            continue
        data['results'].append({
            'taxi': {
...
def notify_dashboard_baecha_cancel(taxi_id: str, dashboard_id:Optional[str]=None):
    data = taxi_id

    if dashboard_id:
        socketio.emit('cancel_baecha', data, to=dashboard_id, namespace='/dashboard')
    else:
        for dashboard in dashboards.keys():
            socketio.emit('cancel_baecha', data, to=dashboard_id, namespace='/dashboard')
...
def notify_taxi_baecha_result(taxi: Taxi,
                              cluster: tuple[np.float64, np.float64],
                              cluster_name: str,
                              demand: float,
                              reason: str):
    lat, lng = cluster
